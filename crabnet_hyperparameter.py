...
mdl = get_model(emb_scaler=1.0)

parameters = ["emb_scaler"]


def crabnet_evaluation_function(parameterization):
    x = np.array([parameterization.get(parameter) for parameter in parameters])
    # In our case, standard error is 0, since we are computing a synthetic function.
    return {"mae": hartmann6(x)}
